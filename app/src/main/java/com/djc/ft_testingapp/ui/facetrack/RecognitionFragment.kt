...
import com.djc.ft_testingapp.ui.facerecord.FaceRecordDirections
...
class RecognitionFragment : Fragment() {
    ...
    private fun initListeners() {
        viewModel.user.observe(super.getViewLifecycleOwner(), {
            if (it!=null){
                if (it.name==""){
                    binding.tVInfo.setText("Name= Unknown, Age= ${it.age}, Gender= ${it.sex}")
                }
                else{
                    binding.tVInfo.setText("Name= ${it.name}, Age= ${it.age}, Gender= ${it.sex}")
                }
            }else{
                binding.tVInfo.setText("Name= Unknown, Age= Unknown, Gender= Unknown")
            }
        })
    }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == 1001) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(super.requireContext(), "Se necesita permiso para usar la cámara", Toast.LENGTH_SHORT).show()
            }
        }
    }

    private fun checkCameraPermission() {
        if (ContextCompat.checkSelfPermission(super.requireContext(), Manifest.permission.CAMERA)
    ...
    private fun initCamera() {
        ...
        val cameraProviderFuture = ProcessCameraProvider.getInstance(super.requireContext())
        cameraProviderFuture.addListener({
            ...
            val preview = Preview.Builder()
                .build()
                .also { it.setSurfaceProvider(binding.pvVRecognition.surfaceProvider) }
            ...
            val imageAnalyzer = ImageAnalysis.Builder()
                ...
                .build()
                .also { analysis ->
                    analysis.setAnalyzer(ContextCompat.getMainExecutor(super.requireContext())) { imageProxy ->
        ...
        }, ContextCompat.getMainExecutor(super.requireContext()))
    }
    ...
    private fun takePhotoAndCropFace() {
        imageCapture?.takePicture(ContextCompat.getMainExecutor(super.requireContext()),
